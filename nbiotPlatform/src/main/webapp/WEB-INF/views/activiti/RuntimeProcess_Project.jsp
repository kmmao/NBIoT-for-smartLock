...
<div class="container" style=" width: 1560px;">
  ...
  <div>
	<p>
		<font size="3" color="black">开始→新建项目→我方项目管理人员审核→客户商务人员审核→结束</font>
	</p>
	...
	<div id="content1" style="display:none;">
	 <div class="panel panel-default">
		<display:table name="${claimedTaskList}" id="curPage" class="table table-striped" sort="external" partialList="false" excludedParams="*"
			...
			<display:column title="任务名称"  property="processName" sortable="true"  style="width:5%;"></display:column>
	 ...
	 <select id="select_page_2" name="select_page_2" onchange="javascript:window.location.href=this.options[this.selectedIndex].value;" style="display:none">
	 	<option value="list.do?claimed_page=1&tabStatus=tab1" selected="selected">1</option>
	 	<c:forEach var="i" begin="2" end="${claimed_maxpage}">
			<option value="list.do?claimed_page=${i}&tabStatus=tab1">${i}</option>
		</c:forEach>
	 </select>
	</div>
	</div>
	<div id="content2" style="display:none;">
	    ...
	    <select id="select_page_3" name="select_page_3" onchange="javascript:window.location.href=this.options[this.selectedIndex].value;" style="display:none">
	 			<option value="list.do?allTask_page=1&tabStatus=tab2" selected="selected">1</option>
	 			<c:forEach var="i" begin="2" end="${allTask_maxpage}">
					<option value="list.do?allTask_page=${i}&tabStatus=tab2">${i}</option>
				</c:forEach>
	 		</select>
		</div>
	  </div>
	...
	<div id="content4" style="display:none;">
	 ...
	 <select id="select_page_1" name="select_page_1" onchange="javascript:window.location.href=this.options[this.selectedIndex].value;" style="display:none">
	 	<option value="list.do?task_no_page=1&tabStatus=tab4" selected="selected">1</option>
	 	<c:forEach var="i" begin="2" end="${task_no_received_maxpage}">
			<option value="list.do?task_no_page=${i}&tabStatus=tab4">${i}</option>
		</c:forEach>
	 </select>
	</div>
  </div>
 		...
 		<div class="modal-content">
            ...
            <div class="modal-body">
            		...
            		<form role="form" id="activitiForm" name="activitiForm" method='post' enctype="multipart/form-data">
            			...
            			<input id="task_verify_processId" name="task_verify_processId" type="hidden">
...
function passTask(){
	...
	var processId = $('#task_verify_processId').val();
	...
	$.ajax({
		...
		success:function(data){
			...
			location.href = "${base}project/list.do?currentTab=" + currentTab;
		}
	});
}
...
function recvTask(obj){
	...
	function finisheTask(obj){
		var id = $(obj).parents('tr').children('td').eq(5).text();
		$.ajax({
	...
	function viewHistory(obj){
		var id = $(obj).parents('tr').children('td').eq(6).text();
		$.ajax({
	...
	function checkTask(obj){
		var id = $(obj).parents('tr').children('td').eq(5).text();
		var processInstanceID = $(obj).parents('tr').children('td').eq(6).text();
		var taskname = $(obj).parents('tr').children('td').eq(9).text();
		var processId = $(obj).parents('tr').children('td').eq(7).text();
		$('#checkLable').html(taskname);
		$('#task_verify').val(id);
		$('#task_verify_instanceId').val(processInstanceID);
		$('#task_verify_processId').val(processId);
